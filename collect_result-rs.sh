...
echo "Graph Name, Max Ref Count, Compression Window, Avg Ref Chain, Bits/Node, Bits/Link" > results_rs_$graph_name.csv
...
for compression_window in "${compression_windows[@]}"; do
    for max_ref_count in "${max_ref_counts[@]}"; do
        ...
        # Save results to CSV
        echo "$graph_name, $max_ref_count, $compression_window, $avg_ref_chain, $bits_per_node, $bits_per_link" >> results_rs_$graph_name.csv
